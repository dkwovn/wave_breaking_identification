...
!设置结点值
		...
		p%val=val(q)
		!print*,'***receive:',p%x,p%y,trace(p%x,p%y)%x,trace(p%x,p%y)%y
		!trace(p%x,p%y)=1
		mode(p%x,p%y)=dr_mode
		if(p%x==node%x.and.p%y==node%y)then
			p%val=-541
			!if(delta_x>x_num-1)then
			if(delta_x>0)then 
...
!		grid(p%x,p%y)=-1                    !这个限制是，对相同的等值线值，已经完成的等值线的格点，不允许被另一条等值线使用
...
if(line_cnt>=1.and.grd_cnt(max)>=x_num)then
	...
	!		if(associated(p,p%next%prev)) print*,'point to node'
			p=>p%next
		!	if(.not.associated(p%prev))print*,'prev does not exist',p%x,p%y
			deallocate(p%prev)
