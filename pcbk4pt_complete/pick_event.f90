...
!p_iso是滑动指针，iso_hd指向读取数据时的头，p_st，p_end分别指向判定为breaking的一段等值线的头和尾
!pq, a moving pointer, pointing to qualified events, namely what will be writen into the file
!pq_hd,the head pointer of the chain of qualified events 
!p,p_hd are moving pointer and head pointer for original events
...
!	print*,year
	...
	! v0 version is with reduced lz and only for illustration
	write(file_obj,"('/home/cliu/data/rwbsm/era/rwbsm/','RWB.database.cmplt.',i2.2,'.bin')")year  !参数
	!file_obj = 	'/home/cliu/data/gfdl_dynamic_core/clara_bl_jet_A5/rwbsm/RWB.database.cmplt.bin'
!	fawbfile = 	'/home/cliu/data/gfdl_dynamic_core/clara_bl_jet_A5/rwbsm/fawb.cmplt.bin'
!	fcwbfile = 	'/home/cliu/data/gfdl_dynamic_core/clara_bl_jet_A5/rwbsm/fcwb.cmplt.bin'
!	sawbfile = 	'/home/cliu/data/gfdl_dynamic_core/clara_bl_jet_A5/rwbsm/sawb.cmplt.bin'
!	scwbfile = 	'/home/cliu/data/gfdl_dynamic_core/clara_bl_jet_A5/rwbsm/scwb.cmplt.bin'
...
!		p=>p_hd
		...
		do q=1,11   !开始每一条等值线上breaking的检测
			!写grid.bin
			!print*,'q=',q
!			open(unit=13,file='d:/data/unformatted/grid.bin',form='unformatted') !参数
!			grid=0
!			call gen_grid(pv,grid,pv_val(q))
!			write(13)grid*1.0
!			close(13)
...
!				read(*,*)something
			...
			iso_end=>p_iso
!			do while(1)
!				print*,p_iso%x,p_iso%y
!				if(.not.associated(p_iso,iso_hd))then
!					p_iso=>p_iso%prev
!				else
!					exit
!				end if
...
!						print*,'::',p_st%x,p_st%y,p_end%x,p_end%y,p%ac_type
						p%val=val(q)							
						p%lz=0
...
!		print*,'number of events:',event_cnt
		p%next=>null()
!		print*,'the answer:',associated(p%next)
!		stop
		...
		do 
			...
			!if(p%lz<=lz_lmt.or.p%lc1<=lc_lmt)then
		...
		tt=0		
!		pq=>pq_hd			!指针pq负责新的qualified的event链
		do 
			tt=tt+1
			!print*,'tt=',tt
			!if(associated(p_hd).eqv..FALSE.)then
...
!				print*,p%x,p%y,p%pv,p%lc,p%lz,p%ac_type
...
!			print*,lbin
!			stop
!			print*,'big events:',cnt
!print*,'biggest now:',ps%x,ps%y
...
!			pq=>pq%next
!			print*,'cnt=',cnt
!			print*,'biggest:',pq%x,pq%y,pq%area,pq%lc,pq%lz
...
!				print*,p%x,p%y,p%pv
			...
			!print*,'we have ',cnt,' rwb in p chain currently'
			...
			do 
				...
				!=== end A ===
				!if(distance(p%x1,p%y1,pq%x1,pq%y1)<1.5*pq%lc1.or.distance(p%x1,p%y1,pq%x1,pq%y1)<1.5*pq%lz.and.p%ac_type==pq%ac_type)then
				if(flag==1)then
					...
					!==================================
					!print*,'deleted one:',p%x,p%y,distance(p%x,p%y,pq%x,pq%y)
					if(associated(p,p_hd))then
						if(associated(p%next))then
							p_hd=>p%next
						else
						...
						rdc_cnt=rdc_cnt-1
						!p%prev%next=>null()     !modified: interpret this line
...
!		deallocate(pq%next)
		pq=>pq_hd
		do 
			!if(associated(pq).eqv..FALSE.)then
			if(.not.associated(pq))then
				exit
			end if
			!write(11)pq%x1,pq%y1,t,pq%ac_type,pq%area1,pq%lc1,pq%lz,pq%val  !write at every t 
			...
			if(pq%ac_type==1)then
				do i=1,nx
					do j=1,ny
						!cov1(i,j)=cov1(i,j)+pq%coverage(i,j)
						if(pq%coverage(i,j)==1) cov1(i,j)=1
						if(pq%coverage(i,j)==2) seg1(i,j)=1
						!if(pq%coverage(i,j)/=0) seg1(i,j)=pq%coverage(i,j)
					end do
				end do
			else if(pq%ac_type==2)then
				do i=1,nx
					do j=1,ny
						!cov2(i,j)=cov2(i,j)+pq%coverage(i,j)
...
!			write(15,*)pq%x,pq%y,t,pq%ac_type,pq%area,pq%lc,pq%lz,pq%val 
			pp_tmp=>pq	!modified
			pq=>pq%next
			deallocate(pp_tmp) !modified
			rdc_cnt=rdc_cnt-1
